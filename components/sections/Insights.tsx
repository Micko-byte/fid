...
export default function Insights() {
  ...
  return (
    <section id="insights" style={{ backgroundColor: "#f7ecc4", paddingTop: "clamp(5.5rem,12vw,11rem)", paddingBottom: "clamp(5.5rem,12vw,11rem)", position: "relative" }} onMouseMove={onMove}>
      <div ref={ref} style={{ maxWidth: "1320px", margin: "0 auto", paddingLeft: "clamp(1.5rem,5vw,6rem)", paddingRight: "clamp(1.5rem,5vw,6rem)" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: "1.5rem", marginBottom: "clamp(2.5rem,5vw,4rem)" }}>
          <div>
            <motion.span initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}
              ...
              <span style={{ width: "26px", height: "1px", background: "#5B0E14", opacity: 0.6 }} /> Our Thinking
            </motion.span>
          ...
          <Link href="/insights" style={{ fontFamily: "var(--font-body)", fontSize: "0.74rem", letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(26,26,26,0.5)", textDecoration: "none", transition: "color 0.3s" }}
            onMouseEnter={(e) => (e.currentTarget.style.color = "#5B0E14")} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(26,26,26,0.5)")}>
            All articles →
          </Link>
        </div>
        ...
        <div>
          {articles.map((a, i) => (
            <motion.a key={i} href={a.href} target="_blank" rel="noopener noreferrer" data-cursor="Read"
              ...
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1.5rem", padding: "clamp(1.3rem,2.6vw,2.1rem) 0", borderTop: "1px solid rgba(91,14,20,0.18)", textDecoration: "none", position: "relative" }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: "clamp(1rem,2vw,2rem)", position: "relative", zIndex: 1 }}>
                ...
                <h3 style={{ fontFamily: "var(--font-heading,'Oswald')", fontWeight: 500, fontSize: "clamp(1.2rem,2.6vw,2.1rem)", lineHeight: 1.04, letterSpacing: "-0.01em", color: hovered === i ? "#5B0E14" : "#1a1a1a", transition: "color 0.3s, transform 0.4s", transform: hovered === i ? "translateX(12px)" : "translateX(0)", textTransform: "uppercase" }}>
                  {a.title}
                </h3>
              </div>
              <ArrowUpRight size={20} weight="light" color={hovered === i ? "#5B0E14" : "rgba(26,26,26,0.3)"} style={{ flexShrink: 0, transition: "transform 0.3s, color 0.3s", transform: hovered === i ? "translate(3px,-3px)" : "none" }} />
            </motion.a>
          ))}
          <div style={{ borderTop: "1px solid rgba(91,14,20,0.18)" }} />
        </div>
      </div>
