...
export default function Marquee() {
  ...
  return (
    <div className="marquee-strip" style={{ overflow: "hidden", backgroundColor: BG, borderTop: "1px solid rgba(252,156,68,0.16)", borderBottom: "1px solid rgba(252,156,68,0.16)", padding: "1.35rem 0", position: "relative" }}>
      ...
      <motion.div style={{ display: "flex", width: "max-content", alignItems: "center", x: reduce ? 0 : x, skewX: reduce ? 0 : skew, willChange: "transform" }}>
        {track.map((item, i) => (
          <span key={i} className="marquee-item" style={{ display: "inline-flex", alignItems: "center", gap: "0.95rem", padding: "0 2.4rem" }}>
            <span style={{ fontFamily: "var(--font-body)", fontSize: "0.6rem", fontWeight: 600, letterSpacing: "0.14em", color: "#FC9C44", fontVariantNumeric: "tabular-nums", opacity: 0.85 }}>{item.num}</span>
            ...
            <span aria-hidden style={{ width: "6px", height: "6px", flexShrink: 0, transform: "rotate(45deg)", border: "1px solid #C7AC9F", opacity: 0.5 }} />
          </span>
        ))}
      </motion.div>

      <style>{`.marquee-item:hover .mq-txt { color: #FC9C44 !important; }`}</style>
    </div>
  );
}
