...
export default function Marquee() {
  ...
  return (
    <div className="marquee-strip" style={{ overflow: "hidden", backgroundColor: BG, borderTop: "1px solid rgba(241,225,148,0.16)", borderBottom: "1px solid rgba(241,225,148,0.16)", padding: "1.35rem 0", position: "relative" }}>
      ...
      <motion.div style={{ display: "flex", width: "max-content", alignItems: "center", x: reduce ? 0 : x, skewX: reduce ? 0 : skew, willChange: "transform" }}>
        {track.map((item, i) => (
          <span key={i} className="marquee-item" style={{ display: "inline-flex", alignItems: "center", gap: "0.95rem", padding: "0 2.4rem" }}>
            <span style={{ fontFamily: "var(--font-body)", fontSize: "0.6rem", fontWeight: 600, letterSpacing: "0.14em", color: "#F1E194", fontVariantNumeric: "tabular-nums", opacity: 0.85 }}>{item.num}</span>
            <span className="mq-txt" style={{ fontFamily: "var(--font-heading,'Oswald')", fontWeight: 500, fontSize: "clamp(0.95rem,1.5vw,1.25rem)", letterSpacing: "0.04em", textTransform: "uppercase", color: "#F5F2EC", whiteSpace: "nowrap", transition: "color 0.4s" }}>{item.label}</span>
            <span aria-hidden style={{ width: "6px", height: "6px", flexShrink: 0, transform: "rotate(45deg)", border: "1px solid #D9AB88", opacity: 0.5 }} />
          </span>
        ))}
      </motion.div>

      <style>{`.marquee-item:hover .mq-txt { color: #F1E194 !important; }`}</style>
    </div>
  );
}
